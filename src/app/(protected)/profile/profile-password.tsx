...
function calcStrength(password: string): Strength {
  ...
  if (score <= 1)
    return { label: "ZAYIF", color: colors.error.DEFAULT, ratio: 0.25 };
  if (score === 2)
    return { label: "ORTA", color: colors.warning.DEFAULT, ratio: 0.5 };
  if (score === 3)
    return { label: "İYİ", color: colors.info.DEFAULT, ratio: 0.75 };
