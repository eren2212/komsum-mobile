...
function ListingCard({ item }: { item: DtoListing }) {
  ...
  return (
    ...
    >
      {/* 1. Resim Alanı */}
      <View className="relative w-full bg-slate-50" style={{ height: CARD_W }}>
        {item.imageUrl ? (
          <Image
...
export default function VerAlScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-secondary" edges={["top"]}>
      ...
      {/* ── Gerekli Kişiler (Ustalar) Bölümü ── */}
      <View className="bg-surface px-6 pt-1 pb-3">
        <TouchableOpacity
          onPress={() => router.push("/service-provider/list")}
          activeOpacity={0.85}
          className="flex-row items-center rounded-[18px] p-4 bg-white border border-neutral-100"
        ...
        >
          ...
          <View className="flex-1">
            <Text className="text-[15px] font-extrabold text-neutral-800 mb-0.5">
              Gerekli Kişiler
            </Text>
            <Text className="text-[12px] text-neutral-400" numberOfLines={1}>
              Öğretmen, tesisatçı, bakıcı… yakınındaki ustaları bul
            </Text>
          </View>
          <View
