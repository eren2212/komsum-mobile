...
function UstaSkeleton() {
  return (
    <SafeAreaView className="flex-1 bg-neutral-50" edges={["top"]}>
      <StatusBar backgroundColor="#6200EE" barStyle="dark-content" animated />
      ...
      <View className="flex-1 bg-white px-6 pt-8 rounded-t-[32px] border-t border-neutral-100">
...
// ─── Boş Durum – Profil Yok ──────────────────────────────────────────────────

function UstaEmptyState({ onCreatePress }: { onCreatePress: () => void }) {
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top", "bottom"]}>
      ...
      <View className="flex-1 items-center justify-center px-8">
        <View className="relative items-center justify-center mb-10">
          ...
          <View className="w-[100px] h-[100px] bg-primary-100 rounded-[32px] items-center justify-center rotate-3">
            <Ionicons name="construct" size={48} color={colors.primary.DEFAULT} />
          </View>
        ...
        <Text className="text-[15px] text-neutral-500 text-center leading-6 mb-12 px-2">
          Öğretmen, su tesisatçısı, bakıcı… Hangi işi yapıyorsan komşularının
          sana kolayca ulaşması için hemen ücretsiz profilini oluştur.
        </Text>
        ...
        <View className="flex-row items-center gap-2 bg-neutral-50 px-5 py-3.5 rounded-2xl border border-neutral-100">
          ...
          <Text className="text-[13px] font-medium text-neutral-600 flex-1">
            Profiliniz admin onayından geçtikten sonra mahallenizde görünür olur.
          </Text>
        </View>
      </View>
    </SafeAreaView>
...
function ManageRow({
  ...
  return (
    <View>
      ...
      >
        <View className="w-[44px] h-[44px] rounded-lg bg-primary/20 items-center justify-center mr-4">
...
function UstaProfile({
  profile,
  onRefresh,
  refreshing,
}: {
  profile: DtoServiceProvider;
  onRefresh: () => void;
  refreshing: boolean;
}) {
  ...
  const { mutate: deleteProfile, isPending: isDeleting } = useMutation({
    mutationFn: serviceProviderApi.deleteMyServiceProviderProfile,
    onSuccess: () => {
      queryClient.setQueryData(["myServiceProviderProfile"], null);
  ...
  const handleDeletePress = () => {
    Alert.alert(
      "Profili Kapat",
      "Usta profilinizi tamamen kapatmak istediğinize emin misiniz? Bu işlem geri alınamaz.",
      [
  ...
  return (
    <SafeAreaView className="flex-1 bg-secondary" edges={["top"]}>
      <StatusBar backgroundColor="#6200EE" barStyle="light-content" animated />
      <ScrollView
      ...
      >
        ...
        <View className="px-6 pt-6 pb-12 relative overflow-hidden">
          <View className="absolute top-[-50px] right-[-50px] w-48 h-48 bg-primary-DEFAULT rounded-full opacity-20" />
          <View className="absolute bottom-[-20px] left-[-20px] w-32 h-32 bg-purple-500 rounded-full opacity-10" />
          ...
          <View className="flex-row items-start justify-between mb-6">
            <View className="bg-white/10 px-3 py-1.5 rounded-lg border border-white/20 self-start">
              <Text className="text-white/90 text-[12px] font-bold uppercase tracking-widest">
                {SERVICE_CATEGORY_LABELS[profile.category]}
              </Text>
            </View>
          ...
          <View className="flex-row items-center gap-2 opacity-80 mb-3">
          ...
          <View className="flex-row flex-wrap gap-2">
            ...
            >
              <Ionicons
                name={profile.available ? "ellipse" : "ellipse-outline"}
                size={10}
                color={profile.available ? "#34D399" : "#CBD5E1"}
              />
              <Text
                className="text-[12px] font-bold"
                style={{ color: profile.available ? "#34D399" : "#CBD5E1" }}
              >
                {profile.available ? "İş alıyor" : "Müsait değil"}
              </Text>
            </View>

            {profile.experienceYears != null && (
              <View className="flex-row items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-full border border-white/20">
        ...
        <View className="flex-1 bg-white px-5 pt-8 rounded-t-[36px] -mt-6">
          ...
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => router.push("/service-provider/list")}
            className="bg-primary rounded-[20px] p-5 mb-8 flex-row items-center justify-between"
          >
            <View className="flex-row items-center gap-3">
              ...
              <View>
                <Text className="text-white text-[16px] font-bold mb-0.5">
                  Gerekli Kişiler Rehberi
                </Text>
                <Text className="text-white/80 text-[12px] font-medium">
                  Mahallendeki diğer ustaları keşfet
                </Text>
              </View>
            </View>
          ...
          <View className="mb-2 px-2">
            <ManageRow
              ...
              onPress={() => router.push("/service-provider/edit")}
            />
            <ManageRow
          ...
          <View className="mb-8 px-2 border border-error/10 bg-error/5 rounded-3xl">
            <ManageRow
              icon="trash-outline"
              label={isDeleting ? "Kapatılıyor..." : "Usta Profilini Kapat"}
...
export default function UstaScreen() {
  const router = useRouter();
  ...
  if (!myProfile) {
    return (
      <UstaEmptyState
        onCreatePress={() => router.push("/service-provider/create")}
      />
    );
  }
